                  ...
                  <form action="{{ route('team.update', $team->slug) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-6 gap-6">
                      <div class="col-span-6">
                        ...
                        <div class="flex justify-center px-6 pt-5 pb-6 mt-1 border-2 border-gray-300 border-dashed rounded-md">
                          <div class="space-y-1 text-center">
                            @if ($team->logo)
                              <img id="img-preview" src="{{ asset('storage/' . $team->logo) }}" class="mx-auto mb-4 rounded-sm" alt="Image Preview" style="max-width: 100%; max-height: 200px;" />
                            @else
                            ...
                            <div class="flex justify-center text-sm text-gray-600">
                              <label for="file-upload" class="relative font-medium text-center text-indigo-600 bg-white rounded-md cursor-pointer focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:text-indigo-500">
                                ...
                                <input type="hidden" name="oldImage" value="{{ $team->logo }}">
                                <input id="file-upload" name="logo" type="file" class="sr-only" onchange="previewImage()" />
                              </label>
                      ...
                      <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="text" name="nama_team" id="nama_team" autocomplete="given-name" class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" value="{{ old('nama_team', $team->nama_team) }}"/>
                      </div>
                      ...
                      <div class="col-span-6 sm:col-span-3">
                        ...
                        <div class="flex mt-1 rounded-md shadow-sm">
                          ...
                          <input type="number" name="resource" id="resource" class="flex-1 block w-full min-w-0 border-gray-300 rounded-none rounded-r-md focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('resource', $team->resource) }}">
                        </div>
                      </div>
                        ...
                        <select id="user_id" name="user_id" class="block w-full py-2 pl-3 pr-10 mt-1 text-base border-gray-300 rounded-md focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                          @foreach ($users as $user)
                            @if ($user->role == "manager")
                              @if (old('user_id', $team->user->id) == $user->id)
